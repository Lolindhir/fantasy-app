# Trade Decision Record – 2026-08-18 Jan / Antonio Williams

## Status
- Record origin: `reconstructed`
- Status: `draft`
- Reconstruction confidence: **5/5**
- Outcome firewall: bestätigt

## Ziel
**Einen bereits im Rookie Draft verpassten Conviction-Target nachträglich holen und dafür exakt bis zum vorher festgelegten Maximum von FA 3.04 gehen.**

## Exakte Transaktion
Robert erhielt:
- **Antonio Williams**

Robert gab ab:
- **2026 FA 3.04**

## Antonio war ein echter Target
Antonio war kein nachträglich entdeckter Buy-Young:
- Robert wollte ihn bereits im Rookie Draft.
- An 3.03 wurde Chris Bell nur knapp vor Antonio gewählt.
- Robert ärgerte sich deutlich, als Jan Antonio an 3.05 nahm.
- Danach blieb Antonio ein konkretes Tradeziel.

Stoned Lack stützte diese Conviction bereits vor dem Rookie Draft sehr deutlich: Antonio wurde als auffälliger Sleeper/Favorite und mit einem klaren frühen Rollen-/WR2-Pfad beschrieben.

## Preis
Der Buy-Young-Plan vom 12.08.2026 hatte den Preis bereits vor der Verhandlung definiert:
- Wunschpreis: FA 4.04
- Fair: 4.04 + 5.04
- **Schmerzgrenze / Hard Ceiling: FA 3.04 straight**
- 3.04 + meaningful add: nein
- FA 2.04: nein

Jan verlangte genau FA 3.04. Robert akzeptierte ohne weiteres Feilschen.

Roberts heutige Erinnerung passt dazu:
- **3.04 fühlte sich fair an**
- **3.04 war das Hard Ceiling**
- allenfalls ein sehr kleiner später Future-Late-Pick wäre noch denkbar gewesen

## Damaliger Rollen-Case
Roberts Wissensstand beim Trade war:
- Terry McLaurin als klarer WR1
- Antonio mit einem plausiblen bzw. relativ klaren Pfad zum WR2
- starke Route-/Versatility-Signale
- positive Stoned-Lack-Einschätzung

Dieser Wissensstand ist für den historischen Decision Record maßgeblich.

### Wichtig: übersehene Information
Stefon Diggs war bereits am **07.08.2026** nach Washington gegangen, elf Tage vor dem Trade.

Robert wusste das am 18.08. **nicht**.

Das wird ausdrücklich **nicht rückwirkend in die ursprüngliche These hineingeschrieben**. Der ursprüngliche Record muss abbilden, was Robert damals tatsächlich wusste und glaubte. Die Diggs-Information ist stattdessen ein sauber dokumentierter späterer Process-Review-Punkt: Eine relevante öffentlich verfügbare Information war beim Trade nicht in Roberts Informationssatz enthalten.

## Roster-/Cut-Kontext
Robert hatte zum Tradezeitpunkt bereits **ein übervolles Roster** und dachte aktiv darüber nach, wen er später cutten müsste.

Das wirkte in zwei Richtungen:
- **Gegen den Trade:** Antonio war noch ein weiterer junger WR in einem ohnehin tiefen Roster und erhöhte den Konkurrenzdruck um Rosterplätze.
- **Für den Trade:** Robert gab mit FA 3.04 einen späteren zusätzlichen Zugang ab. Er musste also im Free-Agent-Draft **einen Spieler weniger aufnehmen**, was einen späteren Cut-Konflikt vermied.

Diese Unsicherheit betraf deshalb weniger Antonio als Spieler, sondern die Frage, ob ein weiterer WR-Zugang für das Gesamtportfolio optimal war.

## Commanders-/Daniels-Bias
Der Commanders-/Jayden-Daniels-Aspekt war laut Robert kein wesentlicher Grund, mehr zu bezahlen.

Er wurde eher in der Kommunikation mit Jan als nachvollziehbare Begründung verwendet. Die eigentliche Zahlungsbereitschaft kam aus:
- Rookie-Draft-Conviction
- Antonio als verpasster Wunschspieler
- wahrgenommenem Rollenpfad
- Stoned-Lack-Signal

## Vorgeschlagener Decision State
- **Decision Confidence: 5/5**
- **Outcome Uncertainty: 3/5**
- **Gut Feeling: very_positive**
- **Decision Margin: clear**
- **Preisgefühl: fair**
- **Execution vs Ceiling: at**

### Warum Confidence 5?
Nach der geschärften Definition geht es um die Überzeugung in genau diese Konstruktion zu genau diesem Preis. FA 3.04 war schon vor Jans Forderung als Hard Ceiling definiert. Als Jan genau diesen Preis nannte, war die Entscheidung klar.

### Warum Uncertainty 3?
Antonio blieb ein Rookie mit Entwicklungsrisiko. Zusätzlich war das Roster bereits übervoll, sodass offen war, ob ein weiterer junger WR für das Gesamtportfolio optimal war. Gleichzeitig half der Trade sogar etwas gegen den Cut-Druck, weil FA 3.04 als späterer zusätzlicher Roster-Zugang wegfiel. Das ergibt echte, aber noch **moderate** Unsicherheit – daher 3/5 statt 2/5 oder 4/5.

## Noch offen vor Seal
Bestätigung oder Anpassung von:
**5/5 Confidence · 3/5 Outcome Uncertainty · very_positive · clear · fair · at ceiling**

## Seal
Noch nicht versiegelt.
