...
class FreeAgentMovementDatasetTests(unittest.TestCase):
    ...
    def test_scoring_comes_from_canonical_league_only(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            source = result["source"]["canonical_league_scoring"]
            self.assertEqual(source["canonical_league_id"], "test")
            self.assertEqual(source["season"], 2026)
            self.assertEqual(source["path"], "source-data/leagues/test/seasons/2026/league.json")
    ...
    def test_repository_canonical_scoring_matches_legacy_app_scoring(self) -> None:
        ...
        scoring, source = MODULE.load_canonical_league_scoring(root, config)
        legacy = json.loads((root / "public/data/League.json").read_text(encoding="utf-8"))["ScoringType"]
        self.assertEqual(scoring, legacy)
        self.assertEqual(source["canonical_league_id"], "nfl-reise")
        self.assertEqual(source["season"], 2026)
