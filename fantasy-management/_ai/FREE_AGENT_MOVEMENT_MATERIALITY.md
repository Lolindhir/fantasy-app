...
The materiality-contract fingerprint includes the explicit positive integer `materiality_contract.version`, comparison windows, resolved materiality thresholds, replacement rules, cross-signal rules, activity thresholds and the configured team-source migration comparison policy. Any semantic materiality-code change that is not already represented by those resolved values **must bump `materiality_contract.version`**. Logging, formatting and performance-only changes must not bump the version.

Checkpoint 6Z.1 introduces `suppress_nfl_team_change_when_source_contract_differs`: when the current and previous Free-Agent player records declare different `nfl_team_source` contracts, only the `nfl_team_change` structural edge is suppressed for that comparison. Injury, role and activity comparisons continue normally. Once both records declare the same team-source contract, a real NFL-team change is again emitted as the existing high-severity `team_transaction` edge.

The evidence fingerprint includes the current Free-Agent and Player input fingerprints, evaluation date/current and baseline ranking-history identities, active Canonical League scoring evidence, source-catalog content and Movement history/quality context. Canonical scoring evidence is identified by CanonicalLeagueID, current season and source path, while its content fingerprint is calculated from `ScoringSettings` only. Unrelated League-state changes therefore do not create Movement evidence drift. Previous-Free-Agent rollover is intentionally excluded because structural Day-over-Day changes are edge events; rolling that baseline forward must not make otherwise identical current evidence appear new.
