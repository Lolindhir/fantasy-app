...
- `rank_ecr` ist die von FantasyPros veröffentlichte finale Konsensreihenfolge. Sie ist nicht garantiert innerhalb von `rank_min` und `rank_max`, weil FantasyPros diese Werte als getrennte Expertenstatistiken liefert und die genaue Beziehung im Payload nicht dokumentiert ist. Solche Fälle werden als Diagnose in `metadata.json` dokumentiert und nicht als fehlerhafter Payload verworfen.
