---
type: podcast_raw_manifest
scope: fantasy-management
source_id: stoned-lack
episode_id: sl_0570
episode_number: 570
title: "Rookie WR Ranking Fantasy Football [2026]"
published_date: 2026-05-04
processed_date: 2026-07-10
language: de
raw_transcript_status: split_parts_imported_from_chat
---

# Raw Transcript Manifest – Stoned Lack 570 – Rookie WR Ranking Fantasy Football [2026]

The raw transcript is stored as ordered parts inside this episode package.

Use the following files in numeric order. The ordered concatenation is the raw transcript source for the episode.

## Parts

1. `part01.md`
2. `part02.md`
3. `part03.md`
4. `part04.md`
5. `part05.md`
6. `part06.md`
7. `part07.md`
8. `part08.md`
9. `part09.md`
10. `part10.md`
11. `part11.md`
12. `part12.md`

## Notes

- Do not clean or rewrite raw parts.
- Preserve timestamps, transcription errors, repetitions and off-topic passages.
- `episode.md` summarizes the podcast content only and should not include this technical inventory.
- `index.json` references this manifest and records raw status.
- If a better transcript is imported later, create a versioned raw source instead of overwriting this trace.
