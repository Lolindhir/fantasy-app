...
- [x] Teams-Übersicht und Team-Detail als Salary-zentrierten Franchise-Hub neu gestalten.
  ...
  - Appweiter Team-Dialog: Team Detail darf nicht an `/teams` gekoppelt sein. Einen gemeinsamen appweiten Öffnungspfad vorsehen, sodass derselbe Dialog später aus Teams, Standings, Drafts, Moves, Overview, Player Detail und Salary-&-Trade-Simulator per stabiler Teamreferenz geöffnet werden kann; keine featurelokale Dialogkopie pro Aufrufer.
  ...
  - Draft-Wiederverwendung: `DraftPickTriggerComponent`/`DraftPickPopoverComponent` nicht für Teams kopieren und Teams nicht blind von Drafts-Feature-Interna abhängig machen. Wenn der Team-Tab einen echten Cross-Feature-Consumer erzeugt, nur den generischen Pick-UI-Kern nach `shared` ziehen und Drafts sowie Teams gemeinsam darauf umstellen.
  ...
  - Legacy-Koexistenz: Die heutige salary-zentrierte Teams-Ansicht mit Exclude/Cut-Funktion während der Migration weiterhin über eine Legacy-Route/-Ansicht erreichbar halten. Sie erst entfernen, wenn der Salary-&-Trade-Simulator für diese Szenarien Funktionsparität erreicht hat.
  - Shared Foundation: Vor den neuen Teams-Views gemeinsame Current-/Projected-, Cap-Usage-, Salary-Health-, Salary-by-Position-, Salary-Movement- und Roster/Taxi/IR-Presentation-Logik schaffen beziehungsweise vorhandene Shared-Utilities erweitern. Neue Salary-Komponenten inputs-basiert und simulator-tauglich statt an den Teams-Dialog gebunden bauen.
