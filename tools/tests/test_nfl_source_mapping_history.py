...
from nfl_source_data_lib.common import write_json_if_changed
...
class NflSourceMappingHistoryTests(unittest.TestCase):
    ...
    def test_historical_mapping_replay_does_not_extend_into_current_owner_conflict(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
    ...
    def test_exact_historical_mapping_can_extend_past_nonoverlapping_owner(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_mapping_payload(
                root,
